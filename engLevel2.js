...
function initGame() {
  ...
  const answerIndex = possibleAnswers.indexOf(answerText);
  const options = sentence.options[answerText];

  // Set up sentence fragments
  ...
  message.textContent = "";
  progressTracker.textContent = `Level: ${currentIndex + 1} / ${totalQuestions}`;
  wordArea.innerHTML = "";

  // Create draggable word options
...
function showFinalMessage() {
  if (correctCount >= passThreshold) {
    message.innerHTML = `🎉 Well done! You completed all questions with ${correctCount} / ${totalQuestions} correct answers. You passed!`;
  } else {
    message.innerHTML = `😢 You've completed all questions, but only got ${correctCount} / ${totalQuestions} right. Try again next time!`;
  }
}
